...
*/
	...
	*/
		function buildFilmstrip() {
			...
			// From jQuery 1.9 on, $.browser is no longer defined. For a quick workaround see, https://drupal.org/node/1961178
			var transColor = (navigator.userAgent.match(/msie [6]/i)) ? 'pink' : 'transparent';
		...
		return this.each(function() {
			...
			$('script').each(function(i){
				...
				if(s.attr('src') && s.attr('src').match(/jquery\.galleryview/)){
					...
					theme_path = s.attr('src').split('jquery.galleryview')[0]+'../images/galleryviewthemes/';	
				}
			});
