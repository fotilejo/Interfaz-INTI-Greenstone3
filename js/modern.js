(function () {
  ...
  function initHelpBooleanCloud() {
    var triggers = document.querySelectorAll(".help-boolean-trigger:not([data-boolean-ready])");
    if (!triggers.length) return;

    var cloud = document.getElementById("help-boolean-cloud");
    if (!cloud) {
      cloud = document.createElement("div");
      cloud.id = "help-boolean-cloud";
      cloud.className = "help-boolean-cloud";
      cloud.setAttribute("role", "dialog");
      cloud.setAttribute("aria-modal", "false");
      cloud.setAttribute("aria-hidden", "true");
      cloud.hidden = true;
      cloud.innerHTML =
        '<div class="help-boolean-cloud__head">' +
          '<h3>Operadores booleanos</h3>' +
          '<button type="button" class="help-boolean-cloud__close" aria-label="Cerrar"><i class="fa-solid fa-xmark" aria-hidden="true"></i></button>' +
        '</div>' +
        '<p class="help-boolean-cloud__intro">Sirven para ajustar una busqueda cuando una palabra sola trae demasiados, muy pocos o resultados mezclados.</p>' +
        '<dl class="help-boolean-cloud__list">' +
          '<dt><code class="help-boolean-cloud__operator">AND</code></dt>' +
          '<dd><span class="help-boolean-cloud__use">Combina ideas: exige que aparezcan ambos conceptos.</span><span class="help-boolean-cloud__example">Ejemplo: <code>alimentos AND envases</code></span></dd>' +
          '<dt><code class="help-boolean-cloud__operator">OR</code></dt>' +
          '<dd><span class="help-boolean-cloud__use">Amplia la busqueda: suma sinonimos o variantes posibles.</span><span class="help-boolean-cloud__example">Ejemplo: <code>bioplastico OR biodegradable</code></span></dd>' +
          '<dt><code class="help-boolean-cloud__operator">NOT</code></dt>' +
          '<dd><span class="help-boolean-cloud__use">Excluye un tema: quita resultados que contienen una palabra.</span><span class="help-boolean-cloud__example">Ejemplo: <code>caucho NOT neumaticos</code></span></dd>' +
          '<dt><code class="help-boolean-cloud__operator">&quot;frase exacta&quot;</code></dt>' +
          '<dd><span class="help-boolean-cloud__use">Busca una expresion completa, respetando el orden de las palabras.</span><span class="help-boolean-cloud__example">Ejemplo: <code>&quot;tecnologia de alimentos&quot;</code></span></dd>' +
        '</dl>';
      document.body.appendChild(cloud);
    }

    var closeButton = cloud.querySelector(".help-boolean-cloud__close");

    function closeCloud() {
      cloud.hidden = true;
      cloud.setAttribute("aria-hidden", "true");
      document.querySelectorAll(".help-boolean-trigger[aria-expanded='true']").forEach(function (button) {
        button.setAttribute("aria-expanded", "false");
      });
    }

    function positionCloud(trigger) {
      var rect = trigger.getBoundingClientRect();
      var width = Math.min(460, window.innerWidth - 32);
      var left = Math.min(Math.max(16, rect.left), window.innerWidth - width - 16);
      var height = cloud.offsetHeight || 0;
      var opensAbove = rect.bottom + 10 + height > window.innerHeight - 16 && rect.top > height + 16;
      var top = opensAbove ? rect.top - height - 10 : rect.bottom + 10;
      cloud.classList.toggle("is-above", opensAbove);
    ...
    triggers.forEach(function (trigger) {
      trigger.dataset.booleanReady = "true";
      trigger.setAttribute("aria-expanded", "false");
      trigger.addEventListener("click", function (event) {
        event.preventDefault();
        event.stopPropagation();
        var isOpen = !cloud.hidden && trigger.getAttribute("aria-expanded") === "true";
        if (isOpen) {
          closeCloud();
          return;
        }
        ...
        positionCloud(trigger);
        if (closeButton) closeButton.focus();
      });
    });

    if (closeButton && closeButton.dataset.booleanCloseReady !== "true") {
      closeButton.dataset.booleanCloseReady = "true";
    ...
    if (document.body.dataset.booleanCloudDismissReady !== "true") {
      document.body.dataset.booleanCloudDismissReady = "true";
      document.addEventListener("click", function (event) {
        if (cloud.hidden) return;
        if (cloud.contains(event.target) || event.target.closest(".help-boolean-trigger")) return;
        closeCloud();
      });
      document.addEventListener("keydown", function (event) {
        if (event.key === "Escape") closeCloud();
      });
      window.addEventListener("resize", closeCloud);
      window.addEventListener("scroll", closeCloud, true);
    }
  }

  function getRecordShareData(card) {
    var titleNode = card && card.querySelector(".inti-head h1");
    var title = titleNode ? titleNode.textContent.replace(/\s+/g, " ").trim() : document.title;
    return {
      title: title || "Repositorio Institucional INTI",
      text: title ? "Mira este registro del Repositorio Institucional INTI:" : "Mira este registro en el Repositorio Institucional INTI:",
      url: window.location.href
    };
  }

  function ensureShareFallback() {
    var modal = document.getElementById("record-share-fallback");
    if (modal) return modal;

    modal = document.createElement("div");
    modal.id = "record-share-fallback";
    modal.className = "record-share-modal";
    modal.setAttribute("aria-hidden", "true");
    modal.innerHTML =
      '<div class="record-share-modal__panel" role="dialog" aria-modal="true" aria-labelledby="record-share-title">' +
        '<div class="record-share-modal__head">' +
          '<h2 id="record-share-title"><i class="fa-solid fa-share-nodes" aria-hidden="true"></i> Compartir registro</h2>' +
          '<button type="button" class="record-share-modal__close" data-share-close aria-label="Cerrar"><i class="fa-solid fa-xmark" aria-hidden="true"></i></button>' +
        '</div>' +
        '<div class="record-share-modal__options">' +
          '<button type="button" class="record-share-option record-share-option--whatsapp" data-share-platform="whatsapp"><i class="fa-brands fa-whatsapp" aria-hidden="true"></i><span>WhatsApp</span></button>' +
          '<button type="button" class="record-share-option record-share-option--x" data-share-platform="x"><i class="fa-brands fa-x-twitter" aria-hidden="true"></i><span>X / Twitter</span></button>' +
          '<button type="button" class="record-share-option record-share-option--email" data-share-platform="email"><i class="fa-solid fa-envelope" aria-hidden="true"></i><span>Email</span></button>' +
          '<button type="button" class="record-share-option record-share-option--copy" data-share-platform="copy"><i class="fa-solid fa-link" aria-hidden="true"></i><span>Copiar enlace</span></button>' +
        '</div>' +
      '</div>';
    document.body.appendChild(modal);

    modal.addEventListener("click", function (event) {
      var close = event.target.closest("[data-share-close]");
      var option = event.target.closest("[data-share-platform]");
      if (event.target === modal || close) {
        closeShareFallback();
        return;
      }
      if (option) {
        executeShareFallback(option.getAttribute("data-share-platform"));
      }
    });
  ...
  function openShareFallback(data) {
    var modal = ensureShareFallback();
    modal.dataset.shareTitle = data.title || "";
    modal.dataset.shareText = data.text || "";
    modal.dataset.shareUrl = data.url || window.location.href;
    modal.classList.add("is-open");
    modal.setAttribute("aria-hidden", "false");
    var firstOption = modal.querySelector("[data-share-platform]");
  ...
  function executeShareFallback(platform) {
    var modal = ensureShareFallback();
    var url = modal.dataset.shareUrl || window.location.href;
    var text = modal.dataset.shareText || "Mira este registro en el Repositorio Institucional INTI:";
    var title = modal.dataset.shareTitle || "Repositorio Institucional INTI";
    var encodedUrl = encodeURIComponent(url);
    var encodedText = encodeURIComponent(text);

    if (platform === "whatsapp") {
      window.open("https://api.whatsapp.com/send?text=" + encodedText + "%20" + encodedUrl, "_blank", "noopener");
    } else if (platform === "x") {
      window.open("https://twitter.com/intent/tweet?text=" + encodedText + "&url=" + encodedUrl, "_blank", "noopener");
    } else if (platform === "email") {
      window.location.href = "mailto:?subject=" + encodeURIComponent(title) + "&body=" + encodedText + "%0A" + encodedUrl;
  ...
  async function shareRecord(card) {
    var data = getRecordShareData(card);
    if (navigator.share) {
      try {
  ...
  function authorLink(name) {
    ...
    link.href = url;
    link.innerHTML = '<i class="fa-solid fa-user-circle"></i> ';
  ...
  function colabLink(name) {
    ...
    link.href = url;
    link.innerHTML = '<i class="fa-solid fa-user-edit"></i> ';
    link.appendChild(document.createTextNode(name));
    return link;
  }
  ...
  function parseSemicolonNames(rawText) {
    var rawParts = rawText.split(";").map(function (n) { return n.trim(); }).filter(Boolean);
    var merged = [];
    rawParts.forEach(function (part) {
      if (part.length <= 3 && part === part.toUpperCase() && merged.length > 0) {
  ...
  function enhanceListingAuthors() {
    document.querySelectorAll("#classifiers td, #resultsTable td, #matchdocs td").forEach(function (cell) {
      ...
      if (legacyAuthors) {
        var legacyNames = parseSemicolonNames(legacyAuthors.textContent);
      ...
      while ((node = walker.nextNode())) {
        ...
        if (!match) continue;
        var names = parseSemicolonNames(match[2]);
  ...
  function createRecordActions(card) {
    ...
    actions.querySelector(".record-action--share").addEventListener("click", function () {
      shareRecord(card);
    });
  ...
  function repairFullText(card) {
    card.querySelectorAll(".row-vermas > a:not([data-modernized])").forEach(function (link) {
      ...
      link.addEventListener("click", function (event) {
        ...
        if (label) label.textContent = open ? "Ocultar texto" : "Ver texto plano";
      });
      var label = link.querySelector("span");
      if (label) label.textContent = "Ver texto plano";
    });
  }
  ...
  function isCrossCollectionSearchForm(form) {
    if (!form) return false;
    if (window.location.pathname.indexOf("/collection/") !== -1) return false;
    return Array.from(form.querySelectorAll(".paramLabel")).some(function (label) {
      return label.textContent.toLowerCase().indexOf("colecciones en las cuales") !== -1;
    }) && !!form.querySelector('select[multiple][name$="collection"]');
  }
  ...
  function enhanceQuery() {
    ...
    var isCrossSearch = isCrossCollectionSearchForm(form);
    var intro = document.createElement("section");
    intro.className = "query-intro";
    var searchType = "Búsqueda Simple";
    var formAction = form.getAttribute("action") || "";
    if (isCrossSearch) {
      searchType = "Búsqueda general";
    } else if (formAction.indexOf("FieldQuery") !== -1 || formAction.indexOf("Advanced") !== -1) {
      searchType = "Búsqueda Avanzada";
    } else if (formAction.indexOf("TextQuery") !== -1) {
      searchType = "Búsqueda por Texto";
    }
    intro.innerHTML = isCrossSearch
      ? '<span class="section-kicker">' + searchType + '</span><h1>Buscá en todo el repositorio</h1><p>Elegí una o más colecciones y escribí una palabra, autor, tema o título para consultar el acervo completo.</p>'
      : '<span class="section-kicker">' + searchType + '</span><h1>Buscá dentro de la colección</h1><p>Combiná términos y filtros para precisar el resultado. Para empezar, alcanza con ingresar una palabra significativa.</p>';
    form.parentNode.insertBefore(intro, form);
    form.classList.add("query-modern-form");
    if (isCrossSearch) {
      document.body.classList.add("repo-cross-search-page");
      form.classList.add("query-modern-form--cross");
    }
    var root = form.querySelector(":scope > div");
    if (!root) return;
    /* ==========================================================================
       BÚSQUEDA SIMPLE Y AVANZADA (VERSIÓN 4 - 100% COMPLETA)
       ==========================================================================
       Agrega clases dinámicas (.query-param-row) a las etiquetas y valores
       para permitir que CSS Flexbox organice los componentes en filas y columnas.
       Para la búsqueda avanzada, se identifica la tabla de parámetros y se
       le asigna la clase .query-fields-table.
       ========================================================================== */
    root.querySelectorAll(".paramLabel").forEach(function (label) {
      ...
      if (txt === "Cadena de búsqueda" || txt === "Término a buscar" || txt.indexOf("rmino a buscar") !== -1) {
        label.textContent = "T\u00E9rminos a buscar";
        row.classList.add("query-param-row--main");
      }
      else if (txt === "Hits por página" || txt.indexOf("Resultados por p") !== -1) {
        label.textContent = "Resultados por página";
        row.classList.add("query-param-row--small", "query-param-row--results");
      }
      ...
      else if (txt.indexOf("Ordenar resultados") !== -1) label.textContent = "Ordenar por";
      else if (txt === "Orden inverso") label.textContent = "Invertir orden";
      else if (txtLower.indexOf("colecciones en las cuales") !== -1) {
        label.textContent = "Colecciones";
        row.classList.add("query-param-row--collections");
      }
      else if (txtLower.indexOf("documentos por colecci") !== -1) {
        label.textContent = "Máx. docs/colección";
        row.classList.add("query-param-row--small");
      }
      ...
      // Chips UI for multiple selects
      var multiSelect = value.querySelector("select[multiple]");
      if (isCrossSearch && multiSelect) {
        multiSelect.style.display = "none";
        var chipContainer = document.createElement("div");
        chipContainer.className = "custom-collection-selector";
        var chipPairs = [];
        function refreshCollectionChips() {
          chipPairs.forEach(function (pair) {
            pair.chip.classList.toggle("is-selected", pair.option.selected);
          });
        }
        ...
        Array.from(multiSelect.options).forEach(function (opt) {
          ...
          chip.type = "button";
          chip.className = "collection-chip" + (opt.selected ? " is-selected" : "");
          
          var originalText = opt.text.trim();
          var newText = originalText;
          var upper = newText.toUpperCase();
          if (upper.indexOf("TODAS") !== -1) newText = "TODAS";
          else if (upper.indexOf("INTI-SNRD") !== -1) newText = "INTI-SNRD";
          else if (upper.indexOf("INTI +") !== -1) newText = "INTI +";
          
          chip.textContent = newText;
          chip.title = originalText;
          
          chip.addEventListener("click", function (e) {
            e.preventDefault();
            var isAllOption = opt.value === "all";
            if (isAllOption) {
              Array.from(multiSelect.options).forEach(function (option) {
                option.selected = option === opt;
              });
            } else {
              ...
              var hasSpecificSelection = Array.from(multiSelect.options).some(function (option) {
                return option.value !== "all" && option.selected;
              });
            ...
            refreshCollectionChips();
          });
          chipPairs.push({ option: opt, chip: chip });
          chipContainer.appendChild(chip);
        });
        value.appendChild(chipContainer);
      }
    ...
    root.querySelectorAll(".query-fields-table select option").forEach(function (option) {
      if (option.textContent.trim().toLowerCase() === "todos los campos") {
  ...
  window.exportCart = function(format) {
    ...
    if (format === "ris") {
      ...
      var bom = "\uFEFF";
      text = bom + ["autor;titulo;anio;editor;uri"].concat(rows.map(function(r) {
  ...
  window.updateCartUI = function() {
    ...
    keys.forEach(function(uri) {
      ...
      div.innerHTML = '<div><strong></strong><span></span></div>' +
                      '<button type="button" class="mini-btn danger cart-remove" title="Quitar"><i class="fa-solid fa-times"></i></button>';
      div.querySelector('strong').textContent = rec.title || "Documento";
      div.querySelector('span').textContent = (rec.autor || "Sin autor") + (rec.anio ? " (" + rec.anio + ")" : "");
      div.querySelector('button').onclick = function(e) { removeSaved(uri, e); };
  ...
  function sortCL2YearsDescending() {
    ...
    var table = document.querySelector("#classifiernodelist");
    if (!table || table.dataset.repoCl2YearsSorted === "true") return;
    ...
    var rows = Array.prototype.slice.call(tbody.children).filter(function (row) {
      return row.tagName && row.tagName.toLowerCase() === "tr" && row.querySelector('table[id^="titleCL2."]');
    });
    if (rows.length < 2) return;
    rows.sort(function (a, b) {
      return getClassifierYear(b) - getClassifierYear(a);
    });
    rows.forEach(function (row) { tbody.appendChild(row); });
    table.dataset.repoCl2YearsSorted = "true";
  }

  function getClassifierYear(row) {
    var titleLink = row.querySelector('table[id^="titleCL2."] a');
    var text = titleLink ? titleLink.textContent : row.textContent;
    var match = (text || "").match(/\b(1[5-9]\d{2}|20\d{2})\b/);
