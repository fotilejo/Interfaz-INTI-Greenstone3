...
<xsl:stylesheet version="1.0"
	...
	<xsl:variable name="opt-doc-link-args">p.s=<gsvar:this-service/><xsl:if test="/page/pageResponse/hierarchy and /page/pageRequest/paramList/param[@name ='s1.collection']">&amp;ck=TextQuery</xsl:if>
	</xsl:variable>
	...
	<xsl:template name="displayResults">
	  <xsl:if test="/page/pageResponse/facetList/facet">
	    <xsl:call-template name="displayFacets"/>
	  </xsl:if>
		<div id="resultsArea">
					...
					<tr id="div{@nodeID}" class="document">
						...
						<xsl:call-template name="highlightSnippet">
							...
							<xsl:with-param name="collection" select="@collection"/>
							  <!--<xsl:choose> <xsl:when test="@collection"><xsl:value-of select="@collection"/></xsl:with-param>-->
						</xsl:call-template>
	...
	<xsl:template name="displayFacets">
	...
	<!-- this usually gets overriden in collectionConfig.xml -->
	<gsf:template match="documentNode">
	  <td class="result-source" valign="top">
	    <gsf:link type="document">
	      <gsf:icon type="document"/>
	    </gsf:link>
	  </td>
	  <td class="result-main">
	    <gsf:link type="document">
	...
	<xsl:template name="highlightSnippet">
	  <xsl:param name="nodeHASH"/>
	    ...
	    <xsl:for-each select="/page/pageResponse/highlightSnippetList/highlightSnippet[@nodeID = $nodeHASH]">
	      <xsl:if test="not($collection) or $collection = @collection">
                        <tr>
                        	<td>
                                </td>
                                <td>
	...
	<xsl:template name="queryPage">
		<xsl:for-each select="pageResponse/service">
		  <xsl:variable name="form_action"><xsl:value-of select="$library_name"/>/<xsl:if test="$collName">collection/<xsl:value-of select="$collName"/>/</xsl:if>search/<xsl:value-of select="@name"/></xsl:variable>
		      <form name="QueryForm" method="get" action="{$form_action}">
		      <div>
					...
					<xsl:choose>
						<xsl:when test="/page/pageResponse/hierarchy and paramList/param[@name ='collection']">
						 	<xsl:call-template name="CCSParams"> <!-- cross coll search -->
	...
	<xsl:template name="CCSParams">
	  <xsl:param name="ns" />
	  <xsl:variable name="currentSelection"><xsl:value-of select="util:getFormattedCCSSelection(/page/pageRequest/paramList/param[@name='s1.collection']/@value,/page/pageRequest/paramList/param[@name='s1.group']/@value)"/></xsl:variable>
	  <!-- display the collection param -->
	  <div class="paramLabel">
			<xsl:value-of select="paramList/param[@name='collection']/displayItem[@name='name']/text()" />
		</div>
			...
			<select multiple="" size='10'>
				...
				<xsl:for-each select="/page/pageResponse/hierarchy/*">
					<xsl:call-template name="hierarchy-display">
					  ...
					  <xsl:with-param name="currentSelection" select="$currentSelection"/>
					</xsl:call-template>
				</xsl:for-each>
			</select>
		</div>
		<br class="clear" />
		<!-- display all the other params -->
	...
	<xsl:template name="hierarchy-display">
		...
		<xsl:choose>
		  <xsl:when test="name(.) = 'group'">
		    <xsl:variable name="group-name">group.<xsl:if test="$path"><xsl:value-of select="$path" />.</xsl:if><xsl:value-of select="@name" /></xsl:variable>
				<option>
					<xsl:attribute name="style">
					...
					<xsl:if test="util:csvContains($currentSelection, $group-name)">
					...
					<xsl:for-each select="./*">
						<xsl:call-template name="hierarchy-display">
							...
							<xsl:with-param name="path"><xsl:if test="$path"><xsl:value-of select="$path" />.</xsl:if><xsl:value-of select="../@name" /></xsl:with-param>
							<xsl:with-param name="currentSelection" select="$currentSelection"/>
						</xsl:call-template>
					</xsl:for-each>
				</option>
			...
			<xsl:otherwise>
				<option>
					...
					<xsl:if	test="util:csvContains($currentSelection, @name)">
						<xsl:attribute name="selected">selected</xsl:attribute>
					</xsl:if>
					<xsl:attribute name="value">
	...
	<xsl:template name="prevNextButtons">	
		...
		<xsl:variable name="queryParams">
			<xsl:for-each select="/page/pageRequest/paramList/param">
				<xsl:if test="@name != 'a' and @name != 'sa' and @name != 'c' and @name != 's' and @name != 'rt' and @name != $startPageName and @name != 'qs'">
					<xsl:text>&amp;</xsl:text>
					<xsl:value-of select="@name"/>
					<xsl:text>=</xsl:text>
		...
		<nav class="modern-pagination" aria-label="Navegación de resultados">
			<!-- FILA 1: Botones -->
			<div class="modern-pagination__row-botones">
				<!-- Previous button -->
				<div class="modern-pagination__prev">
					<xsl:text> </xsl:text>
					<xsl:if test="$currentPage != 1">
						<a class="modern-pagination__btn" href="{$library_name}?a=q&amp;sa={/page/pageRequest/@subaction}&amp;c={$collName}&amp;s={/page/pageResponse/service/@name}&amp;rt=rd&amp;{$startPageName}={$currentPage - 1}&amp;qs={/page/pageRequest/paramList/param[@name='qs']/@value}{$queryParams}" title="Anterior">
							<i class="fa-solid fa-chevron-left"><xsl:text> </xsl:text></i>
						</a>
					</xsl:if>
				</div>
				...
				<xsl:if test="$docsPerPage &gt; 0">
					<div class="modern-pagination__pages">
						<xsl:text> </xsl:text>
						<xsl:call-template name="generate-page-links">
							<xsl:with-param name="currentPage" select="$currentPage"/>
							<xsl:with-param name="maxPages" select="ceiling($docMax div $docsPerPage)"/>
							<xsl:with-param name="i" select="1"/>
							<xsl:with-param name="startPageName" select="$startPageName"/>
							<xsl:with-param name="queryParams" select="$queryParams"/>
						</xsl:call-template>
					</div>
				</xsl:if>

				<!-- Next button -->
				<div class="modern-pagination__next">
					<xsl:text> </xsl:text>
					<xsl:if test="$docsPerPage &gt; 0 and ($currentPage * $docsPerPage + 1) &lt; $docMax">
						<a class="modern-pagination__btn" href="{$library_name}?a=q&amp;sa={/page/pageRequest/@subaction}&amp;c={$collName}&amp;s={/page/pageResponse/service/@name}&amp;rt=rd&amp;{$startPageName}={$currentPage + 1}&amp;qs={/page/pageRequest/paramList/param[@name='qs']/@value}{$queryParams}" title="Siguiente">
							<i class="fa-solid fa-chevron-right"><xsl:text> </xsl:text></i>
						</a>
					</xsl:if>
				</div>
			</div>
			...
			<xsl:if test="$docsPerPage &gt; 0">
				...
				<div class="modern-pagination__status">
					<xsl:text>Mostrando registros </xsl:text>
					<strong><xsl:value-of select="$startdoc"/>-<xsl:value-of select="$enddoc"/></strong>
					<xsl:text> de </xsl:text>
					<strong><xsl:value-of select="$docMax"/></strong>
				</div>
			</xsl:if>
			</xsl:if>
			...
			<!-- FILA 3: Ir a página -->
			<xsl:if test="$docsPerPage &gt; 0">
				<form method="get" action="{$library_name}" class="modern-pagination__go">
					<input type="hidden" name="a" value="q"/>
					<input type="hidden" name="sa" value="{/page/pageRequest/@subaction}"/>
					<input type="hidden" name="c" value="{$collName}"/>
					<input type="hidden" name="s" value="{/page/pageResponse/service/@name}"/>
					<input type="hidden" name="rt" value="rd"/>
					<input type="hidden" name="qs" value="{/page/pageRequest/paramList/param[@name='qs']/@value}"/>
					<xsl:for-each select="/page/pageRequest/paramList/param">
						<xsl:if test="@name != 'a' and @name != 'sa' and @name != 'c' and @name != 's' and @name != 'rt' and @name != $startPageName and @name != 'qs'">
							<input type="hidden" name="{@name}" value="{@value}"/>
						</xsl:if>
					</xsl:for-each>
					<span class="modern-pagination__go-label">Ir a página:</span>
					<input type="number" name="{$startPageName}" min="1" max="{ceiling($docMax div $docsPerPage)}" class="modern-pagination__input" required="required"/>
					<button type="submit" class="modern-pagination__go-btn">
						<i class="fa-solid fa-arrow-right"><xsl:text> </xsl:text></i>
					</button>
					<span class="modern-pagination__go-label">de <xsl:value-of select="ceiling($docMax div $docsPerPage)"/></span>
				</form>
			</xsl:if>
		</nav>
		</xsl:if>
	...
	<!-- Recursive template to generate page links -->
	<xsl:template name="generate-page-links">
		<xsl:param name="currentPage"/>
		<xsl:param name="maxPages"/>
		<xsl:param name="i"/>
		<xsl:param name="startPageName"/>
		<xsl:param name="queryParams"/>
		
		<xsl:if test="$i &lt;= $maxPages">
			<!-- Show a limited window of 5 pages -->
			<xsl:variable name="showPage">
				<xsl:choose>
					<!-- Show page if it's within +/- 2 of current page -->
					<xsl:when test="$i &gt;= $currentPage - 2 and $i &lt;= $currentPage + 2">true</xsl:when>
					<!-- Or if it's the very first page -->
					<xsl:when test="$i = 1">true</xsl:when>
					<!-- Or if it's the very last page -->
					<xsl:when test="$i = $maxPages">true</xsl:when>
					<xsl:otherwise>false</xsl:otherwise>
				</xsl:choose>
			</xsl:variable>

			<xsl:if test="$showPage = 'true'">
				<!-- If we are jumping from 1 to a number > 2, add an ellipsis -->
				<xsl:if test="$i = $currentPage - 2 and $i &gt; 2">
					<span class="modern-pagination__ellipsis">...</span>
				</xsl:if>

				<a>
					<xsl:attribute name="class">
						<xsl:choose>
							<xsl:when test="$i = $currentPage">modern-pagination__page active</xsl:when>
							<xsl:otherwise>modern-pagination__page</xsl:otherwise>
						</xsl:choose>
					</xsl:attribute>
					<xsl:attribute name="href">
						<xsl:value-of select="concat($library_name, '?a=q&amp;sa=', /page/pageRequest/@subaction, '&amp;c=', $collName, '&amp;s=', /page/pageResponse/service/@name, '&amp;rt=rd&amp;', $startPageName, '=', $i, '&amp;qs=', /page/pageRequest/paramList/param[@name='qs']/@value, $queryParams)"/>
					</xsl:attribute>
					<xsl:value-of select="$i"/>
				</a>

				<!-- If we are jumping from a number to the last page, add an ellipsis -->
				<xsl:if test="$i = $currentPage + 2 and $i &lt; $maxPages - 1">
					<span class="modern-pagination__ellipsis">...</span>
				</xsl:if>
			</xsl:if>
	...
	<xsl:template name="previousArrowImage">
	  <img src="interfaces/{$interface_name}/images/previous.png"/>
	</xsl:template>
	<xsl:template name="nextArrowImage">
	  <img src="interfaces/{$interface_name}/images/next.png"/>
	</xsl:template>
