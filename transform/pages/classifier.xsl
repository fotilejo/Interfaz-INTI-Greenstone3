...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/page/pageResponse">
    ...
    <script type="text/javascript" src="interfaces/{$interface_name}/js/classifier_scripts.js?v=6.35"><xsl:text> </xsl:text></script>
		...
		<td>
		  <table id="title{@nodeID}"><tr>
		    <xsl:if test="not(/page/pageResponse/format[@type='browse']/gsf:option[@name='turnstyleClassifiers']) or /page/pageResponse/format[@type='browse']/gsf:option[@name='turnstyleClassifiers']/@value='true'">
		      <td class="headerTD">
			<button id="toggle{@nodeID}" type="button" onclick="toggleSection('{@nodeID}');" class="gs-toggle-button" aria-label="Desplegar o contraer">
			  <xsl:if test="classifierNode or documentNode"><xsl:attribute name="class">gs-toggle-button is-open</xsl:attribute></xsl:if>
