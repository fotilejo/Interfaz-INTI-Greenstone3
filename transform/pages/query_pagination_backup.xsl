		...
		<nav class="modern-pagination" aria-label="Navegación de resultados">
			...
			<xsl:if test="$docsPerPage &gt; 0">
				<form method="get" action="{$library_name}" class="modern-pagination__go">
					...
					<input type="number" name="{$startPageName}" min="1" max="{ceiling($docMax div $docsPerPage)}" class="modern-pagination__input" placeholder="Pág." required="required"/>
					<button type="submit" class="modern-pagination__go-btn">Ir</button>
				</form>
			</xsl:if>
		</nav>
		</xsl:if>
