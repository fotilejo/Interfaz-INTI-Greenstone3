...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="gsf:div">
	  <div>
		...
		<xsl:value-of select="."/>
            ...
            <xsl:apply-templates/>
	    <xslt:value-of select="''" /> <!-- to ensure that the div tag does not collapse into one of the form <div/> -->
	  </div>
	</xsl:template>

	<xsl:template match="gsf:dquote"><xslt:text disable-output-escaping="yes">"</xslt:text></xsl:template>
	<!--
	    While double-quotes do map to entities in HTML, technically single quotes don't need to go through
	    this process: they can just be typed literally, and so this means there isn't strictly a need
	    for the following rule, but included for completeness.  A long time ago it might have been that IE
	    did need the character to be entity-mapped -->
	<xsl:template match="gsf:squote"><xslt:text disable-output-escaping="yes">'</xslt:text></xsl:template>	
	...
	<xsl:template match="gsf:image">
		<xslt:variable name="metaName">
			<xsl:choose>
				...
				<xsl:when test="@type = 'cover'">root_hascover</xsl:when>
			</xsl:choose>
		</xslt:variable>
		<xslt:variable name="thisAssocfilepath">					
		  <xslt:choose>
		    ...
		    <xslt:otherwise>
		      <xslt:choose>
			<xslt:when test="(.//metadataList)[last()]/metadata[@name='assocfilepath']">
		  ...
		  <xslt:if test="./metadataList/metadata[@name = $metaName]">
			<img>
				<xslt:attribute name='src'>
					...
					<xslt:choose>
					  <xslt:when test="$metaName = 'root_hascover'">cover.jpg</xslt:when>
					  <xslt:otherwise>
  ...
  <xsl:template match="gsf:rank">
	...
	<xsl:template match="gsf:icon">
		<xsl:choose>
			<xsl:when test="@type='classifier'">
				<i class="fa-solid fa-folder-tree modern-resource-icon" aria-hidden="true"><xsl:text> </xsl:text></i>
			</xsl:when>
			<xsl:when test="@type='web'">
					...
					<xslt:attribute name="class">
						<xslt:choose>
							<xslt:when test="@docType='hierarchy' and @nodeType='root'">
								<xsl:text>fa-solid fa-book modern-resource-icon</xsl:text>
							</xslt:when>
							<xslt:otherwise>
								<xsl:text>fa-regular fa-file-lines modern-resource-icon</xsl:text>
							</xslt:otherwise>
						</xslt:choose>
					</xslt:attribute>
					<xsl:text> </xsl:text>
...
-->
			...
			<xslt:for-each>
				...
				<xslt:if test="{$postest}">
					...
					<xsl:choose>
						...
						<xsl:when test="@highlight"><xslt:call-template name="displayMarkedUpTextAndAnnotations"/>
						</xsl:when>
						<xsl:otherwise>
  ...
  <xsl:template match="gsf:text">
    <xsl:variable name="force">
      <xsl:choose><xsl:when test="@force = 'true'">1</xsl:when><xsl:when test="@force = 'false'">0</xsl:when><xsl:otherwise>0</xsl:otherwise></xsl:choose>
    </xsl:variable>
		<xslt:call-template name="documentNodeText"><xslt:with-param name="force"><xsl:value-of select="$force"/></xslt:with-param></xslt:call-template>
	</xsl:template>
